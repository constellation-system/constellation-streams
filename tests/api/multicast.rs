...
fn test_select_all_succeed() {
    ...
    assert_eq!(parties, vec![0.into(), 1.into(), 2.into()]);
    ...
    assert!(stream.stream(0.into()).failures.is_empty());
    assert!(
        stream
            .stream(1.into())
            .batches
    ...
    assert!(stream.stream(1.into()).failures.is_empty());
    assert!(
        stream
            .stream(2.into())
            .batches
...
fn test_select_multi_subset_succeed() {
    ...
    assert_eq!(parties, vec![0.into(), 1.into()]);
...
fn test_select_all_indef() {
    ...
    assert_eq!(parties, Parties::Some(vec![0.into(), 1.into(), 2.into()]));
...
fn test_select_subset_indef() {
    ...
    assert_eq!(parties, Parties::Some(vec![0.into(), 1.into()]));
...
fn test_select_one_indef() {
    ...
    assert_eq!(parties, vec![0.into(), 2.into()]);
...
fn test_select_indef_retry_succeed() {
    ...
    assert_eq!(parties, vec![1.into(), 2.into()]);
...
fn test_push_frags_all_indef() {
    ...
    > = StreamMulticaster::create(config, &mut ()).expect("Expected success");
    let frags_params = vec![Retry::default(); 3];
    let mut frags: StreamMulticasterFrags<OutboundFrags> =
        StreamMulticasterFrags::from_data(frags_params, vec![0x55; 2048]);
